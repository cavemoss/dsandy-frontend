...
export default function Customer({ info }: Params) {
  ...
  if (info && !isMobile) {
    return (
      <Button variant="ghost" className="pr-1 max-w-50 flex" onClick={() => navStore.push('/account')}>
  ...
  if (info) {
    return (
      <Button onClick={() => navStore.push('/account')} variant="ghost" size="icon" className="relative">
